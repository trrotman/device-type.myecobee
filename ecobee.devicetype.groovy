...
void iterateAction(Object[] args) {
	Integer MAX_TSTAT_BATCH = get_MAX_TSTAT_BATCH()
	def tstatlist = null
	Integer nTstats = 0
	def action = args[0]
	def actionCap = args[0].capitalize()
	def ecobeeType = determine_ecobee_type_or_location(args[1])
	def newArgs
	int j=0
	...
	if (settings.trace) {
	log.debug
	"iterate${actionCap}>ecobeeType=${ecobeeType},about to loop ${data.thermostatCount} thermostat(s)"
	sendEvent name: "verboseTrace", value:
	"iterate${actionCap}>ecobeeType=${ecobeeType},about to loop ${data.thermostatCount} thermostat(s)"
	}
	...
	if (settings.trace) {
	sendEvent name: "verboseTrace", value:
	"iterate${actionCap}>about to call ${action} for ${tstatlist}"
	log.debug "iterate${actionCap}> about to call ${action} for ${tstatlist}"
	}
	"${action}"(tstatlist,newArgs) // passing all arguments after the 2nd one
	tstatlist = Id
	nTstats = 1
	} else {
...
void iterateSetThermostatSettings(tstatType, tstatSettings = []) {
	...
	iterateAction('setThermostatSettings', tstatType, tstatSettings)
}
...
void iterateCreateVacation(tstatType, vacationName, targetCoolTemp,
	...
	iterateAction('createVacation',tstatType, vacationName, targetCoolTemp,
	targetHeatTemp, targetStartDateTime, targetEndDateTime)
}
